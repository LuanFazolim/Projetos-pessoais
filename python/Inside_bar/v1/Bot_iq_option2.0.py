...
def atualizar_csv(paridades, timeframe):
    dados = []
    ...
    if not df.empty:
        df_rotulos = obter_rotulos_para_ml()
        if not df_rotulos.empty:
            df = df.merge(df_rotulos, how='left', left_on='par', right_on='moeda')
            df['sinal'] = df['resultado'].map({'acerto': 'call', 'erro': 'put'})
        with lock:
    ...
    for par in paridades:
        ...
        for i in range(periodo, len(candles)):
            ...
            dados.append({
                ...
                'rsi': feat[0], 'k': feat[1], 'd': feat[2],
                'ema': feat[3], 'macd': feat[4], 'signal': feat[5], 
                'boll_upper': feat[6], 'boll_lower': feat[7],
                'sinal': 'none'
            })
    df = pd.DataFrame(dados)
    if not df.empty:
        ...
        if not df_rotulos.empty:
            ...
            df_rotulos['timestamp'] = df_rotulos['timestamp'].dt.floor('min')

            # Faz o merge com base no par e no timestamp
            df = df.merge(df_rotulos, how='left', left_on=['par', 'timestamp'], right_on=['moeda', 'timestamp'])
            df['sinal'] = df['resultado'].map({'acerto': 'call', 'erro': 'put'})
...
atualizar_csv(paridades, timeframe)
...
try:
    ...
    while True:
        ...
        if not horario_valido():
            print(Fore.YELLOW + "Fora do horário de operação. Aguardando...")
            time.sleep(60)
            continue

       # (...todo o seu código acima permanece inalterado até esta parte...)
        ...
        for par in paridades:
            ...
            closes = [x['close'] for x in candles]  # corrigido aqui
            ...
            # Estratégia base: Bollinger + RSI
            if preco_atual < boll_lower and rsi < 10:
                sinal = 'call'
            elif preco_atual > boll_upper and rsi > 80:
                sinal = 'put'

            if sinal != 'none' and timestamp not in sinais_enviados:
                # Filtro técnico extra: EMA + MACD
                ema = calcular_ema(closes)
                macd, signal_line = calcular_macd(closes)

                if sinal == 'call' and not (preco_atual < ema and macd > signal_line):
                    continue
                if sinal == 'put' and not (preco_atual > ema and macd < signal_line):
                    continue

                # Filtro IA (apenas se disponível e com boa precisão)
                ...
                if ml_pronto and precisao >= 55:
                    ...
                    if ia_sinal != sinal:
                        continue  # IA não confirmou, ignora o sinal
