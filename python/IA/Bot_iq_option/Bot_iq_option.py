...
horarios_bons = [9,10, 11, 12, 14, 15,16,17,18,19,20]
...
def atualizar_csv(paridades, timeframe):
    ...
    for par in paridades:
        ...
        if not candles:
            continue
        for i in range(periodo, len(candles)):
            feat, _, _ = extrair_features(candles[:i+1])
            dados.append({
                'par': par, 'timestamp': candles[i]['from'],
                'rsi': feat[0], 'k': feat[1], 'd': feat[2],
                'ema': feat[3], 'macd': feat[4], 'signal': feat[5], 'sinal': 'none'
            })
    df = pd.DataFrame(dados)
    with lock:
        df.to_csv(csv_file, index=False)
...
try:
    ...
    while True:
        ...
        for par in paridades:
            ...
            if preco_atual < boll_lower and rsi < 30:
                sinal = 'call'
            elif preco_atual > boll_upper and rsi > 70:
                sinal = 'put'
